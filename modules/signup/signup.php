<?php 
    ...
    if(isset($_POST)&& !empty($_POST)){
        $name=stripslashes(mysqli_real_escape_string(trim($_POST['name'])));
        $email=stripslashes(mysqli_real_escape_string(trim($_POST['email'])));
        $pass=stripslashes(mysqli_real_escape_string(trim($_POST['pass'])));
        $conpass=stripslashes(mysqli_real_escape_string(trim($_POST['conpass'])));
        if($pass==$conpass){
    ...
    <body>
        <div class="container">
          <form class="form-signin" role="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <h2 class="form-signin-heading">Welcome To My Kitchen</h2>
            <h3 class="form-signin-heading">Signup for the Home food in town at your doorstep </h3>
